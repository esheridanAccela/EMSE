...
function emailContact(mSubj,mText)   // optional: Contact Type, default Applicant
	{
	var replyTo = "noreply@stlouisco.com";
...
function paymentByTrustAccount(fSeqNbr) //optional: itemCap
  {
	...
	if (capLicenseResult.getSuccess())
		...
		if (!lpArray) return false; //no LPs found
		}
	else
		{
		//no LPs found
	...
	tPAcctResult = aa.trustAccount.getPrimaryTrustAccountID(itemCap,null);
	
	if (tPAcctResult.getSuccess())
	  {
		tAccountID = tPAcctResult.getOutput().toString();
		tAccountID = tAccountID.substring(1,tAccountID.lastIndexOf("]"));
	...
	//create payment
	//presult = aa.finance.makePaymentlocal(p);
	presult = makePaymentlocal(p);
...
function makePaymentlocal(payModel) { 
// Payment process given to us on 1/27/2017 by Accela Cusotmer Support 
var payNum = null; 
var result; 
...
paymentModel.setAmountNotAllocated(payModel.getAmountNotAllocated()); 
...
paymentModel.setAcctID(payModel.getAcctID()); 
try{ 
...
function send311IntegrationErrorEmail(emailText){
	aa.sendEmail("noreply@stlouisco.com", "appdev@stlouisco.com", "", "311 Accela Integration Issue", emailText, "");
}

function updateCapIdAPOFromReference(){ //takes optional capId
	var vCapId = null;
	if (arguments.length > 0){
		vCapId = arguments[0];
	}
	...
	var capParcelArray = aa.parcel.getParcelDailyByCapID(vCapId, null).getOutput();
	
	if(capParcelArray != null){
		for(x in capParcelArray){
			var refParcelModelResult = aa.parcel.getParceListForAdmin(capParcelArray[x].getParcelNumber(), null, null, null, null, null, null, null, null, null);
			
			if(refParcelModelResult.getSuccess()){
				var refParcelModelArray = refParcelModelResult.getOutput();
				
				if(refParcelModelArray.length){
					var refParcelModel = refParcelModelArray[0].getParcelModel();
					var capParcelModelFromRefResult = aa.parcel.warpCapIdParcelModel2CapParcelModel(vCapId, refParcelModel);
					
					if(capParcelModelFromRefResult.getSuccess()){
						var capParcelModelFromRef = capParcelModelFromRefResult.getOutput();
						
						var parcelModelFromRef = capParcelModelFromRef.getParcelModel();
						//truncate book because data is messy
						parcelModelFromRef.setBook(parcelModelFromRef.getBook().substring(0, 8));
						//set UID on incoming capParcel
						capParcelArray[x].getParcelModel().setUID(parcelModelFromRef.getUID());
						
						var attributeUpdateResult = aa.parcel.updateDailyParcelWithAPOAttribute(capParcelModelFromRef);
						if(attributeUpdateResult.getSuccess() != true){
							send311IntegrationErrorEmail("Error updating APO Attributes for parcel: " + attributeUpdateResult.getErrorMessage() + " " + vCapId);
						}
					}else{
						send311IntegrationErrorEmail("Failed to Warp. " + vCapId);
					}
				}else{
					send311IntegrationErrorEmail("No reference parcel. " + vCapId);
				}
			}else{
				send311IntegrationErrorEmail("Failed to get reference parcels. " + vCapId);
			}
		}
	}else{
		send311IntegrationErrorEmail("No cap parcels for capId: " + vCapId);
	}
}

function doHttpPostRequest(username, password, url, body, contentType, charSet){
	...
	var authPrefs = [org.apache.commons.httpclient.auth.AuthPolicy.BASIC];
	client.getParams().setParameter(org.apache.commons.httpclient.auth.AuthPolicy.AUTH_SCHEME_PRIORITY, authPrefs);
        client.getParams().setAuthenticationPreemptive(true);
        client.getState().setCredentials(org.apache.commons.httpclient.auth.AuthScope.ANY, credentials);
    }

    post.setRequestEntity(new org.apache.commons.httpclient.methods.StringRequestEntity(body, contentType, charSet));

	try{
		var httpStatus = client.executeMethod(post);
		...
		//take out aa.prints for production
		aa.print(exception);
		aa.print(post.getResponseBodyAsString());
        return {
            error: true,
            exception: exception
        }
...
function get311Webhook(itemCap){
	var appSpecInfoResult = aa.appSpecificInfo.getByCapID(itemCap);
	if(appSpecInfoResult.getSuccess())
	{
		var appSpecObj = appSpecInfoResult.getOutput();
		for (i in appSpecObj){
			if(appSpecObj[i].getCheckboxDesc() == "Webhook" && appSpecObj[i].getCheckboxType() == "311"){
				return appSpecObj[i].getChecklistComment();
			}
		}
	}
...
function translate311Status(statusName, capType){
	const invStatusDictionary = {
		"OPEN": "Open",
		"CLOSED": "Completed"
	};
	
	const pmcStatusDictionary = {
		"OPEN": "Open",
                "Closed": "Completed",
		"Case Continued by Court": "InProgress",
		"Continues In Violation": "InProgress",
		"Extension": "InProgress",
		"Extension Letter Sent": "InProgress",
		"In Violation": "InProgress",
		"Inspection Pending": "InProgress",
		"Manager Approved Case": "InProgress",
		"Non Bill Vacant Monitor": "InProgress",
		"Notice of Violation Issued": "InProgress",
		"OTV Posted": "InProgress",
		"Pending": "InProgress",
		"Prep for Legal": "InProgress",
		"Prepared for Legal": "InProgress",
		"Sent to Legal": "InProgress",
		"Sent to Vacant Billing": "InProgress",
		"Sent to Vacant Monitor": "InProgress",
		"Manager Denied Case": "InProgress",
		"Violation Closed": "Completed",
		"Void": "Completed",
		"Voided": "Completed",
		"Abated": "Completed",
		"Case Abated": "Completed",
		"No Violation": "Completed",
		"Referral to Agency": "Completed",
		"Investigation Closed": "Completed",
		"Cased Closed by Court": "Completed"
	};
	//use appropriate dictionary based on type
	if(capType == "N_Pres/Code Enforcement/Residential/Property Maintenance"){
		return pmcStatusDictionary[statusName];
	}else if(capType == "PublicWorks/Investigation/Commercial/Building"){
		return invStatusDictionary[statusName];
	}
}

function getPermitStatusHistory(statusCap){
	var statusHistoryResult = aa.cap.getStatusHistoryByCap(statusCap, "PERMIT", null);
	if(statusHistoryResult.getSuccess()){
		return statusHistoryResult.getOutput();
	}
}

function update311Ticket(){//optional capId parameter
	var itemCap;
	...
	var webhook = get311Webhook(itemCap);
	//make sure webhook exists before continuing+
	if(webhook != null && webhook != ""){
		//get cap type from itemCap
		var cap = aa.cap.getCap(itemCap).getOutput();
		var capType = cap.getCapType();
		var currentAccelaStatus = cap.getCapStatus();
		
		//get histories
		var statusHistories = getPermitStatusHistory(itemCap);
		var previousAccelaStatus;
		//get last history, if it exists
		if(statusHistories.length > 0){
			previousAccelaStatus = statusHistories[statusHistories.length - 1];
		}
		//get 311 statuses
		var current311Status = translate311Status(currentAccelaStatus, capType);
		var previous311Status;
		if(previousAccelaStatus != null){
			previous311Status = translate311Status(previousAccelaStatus, capType);
		}

		//compare 311 statuses
		if(current311Status != previous311Status){
			//only make call if 311 status has changed between last two histories
            //set ticket notes only on completed 311 status
            var ticketNotes;
            if(current311Status == "Completed"){
                ticketNotes = "An inspector has taken appropriate action to resolve this issue.";
            }
			var updateBody = "{\"status\": \"" + current311Status + "\", \"ticketNotes\": \"" + ticketNotes + "\"}";
			var response = doHttpPostRequest("accelasvc", "1hT%NMBf7L$Uwe", webhook, updateBody, "application/json", "UTF-8");
            
            //check for failure
            if(response.error || response.status != 200){
                //send email on failure
                send311IntegrationErrorEmail("Error updating 311: " + response.body);
                var emailText = "Failed to update 311 ticket for Accela record " + itemCap.toString() + ". Status of ticket with ID " + webhook.substring(webhook.lastIndexOf("/") + 1) + " needs to be updated to " + current311Status + ".";
                
                aa.sendEmail("noreply@stlouisco.com", "Gateway311@stlouisco.com", "", "Accela 311 Update Failure", emailText, "");
            }
		}
	}
}